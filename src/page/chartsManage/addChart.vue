...
export default {
  ...
  methods: {
    ...
    save: function () { // 保存
      this.chartObj.formName = this.tableName
      this.chartObj.searchs = JSON.stringify(this.searchsData)
      this.chartObj.creater = ''
      this.chartObj.create_date = Util.getCurrentDate()
      ...
      this.$api.post('/crm/ActionFormSelectUtil/Select/insertCharts.do', {jsonStr: jsonStr}, r => {
        if (r.data === 1) {
          this.$Message.success('新增图表成功')
          this.$parent.initChartsData()
          this.$parent.$layer.closeAll()
        } else {
          this.$Message.error('新增图表失败')
        }
      })
    },
